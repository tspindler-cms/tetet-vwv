...
local vwv_mig17f = {
	...
	SFM_Data = {
		aerodynamics = {			
			Cy0					= 0.01,				-- [coeff] Zero AoA lift (0.022?)
			...
			cx_flap				= 0.080,			-- [coeff] Flap drag (0.140 better?)
			cy_flap				= 0.20,				-- [coeff] Flap lift (0.10 better?)
			...
			table_data = {
                ...
                -- LOW SPEED
                { 0.0,  0.025,  0.060, 0.180, 0.02, 0.400,  18.0,  1.15 }, 
                { 0.1,  0.025,  0.060, 0.180, 0.02, 0.400,  18.0,  1.15 },
                { 0.2,  0.025,  0.060, 0.180, 0.02, 0.850,  18.0,  1.15 },

                -- CRUISE TRANSITION
                { 0.3,  0.024,  0.058, 0.185, 0.05, 1.400,  18.0,  1.15 },
                { 0.4,  0.024,  0.055, 0.190, 0.06, 1.600,  18.0,  1.15 },
                { 0.5,  0.024,  0.052, 0.195, 0.08, 1.600,  17.5,  1.12 },
                
                -- ANTI-FLOAT / FIGHTING
                { 0.6,  0.024,  0.050, 0.200, 0.10, 1.500,  17.0,  1.08 },
                { 0.7,  0.025,  0.050, 0.210, 0.12, 1.100,  16.5,  1.02 },

                -- TRANSONIC ENTRY (Mach 0.80)
                { 0.8,  0.027,  0.049, 0.220, 0.15, 0.600,  15.0,  0.94 }, 
                
                -- HIGH SPEED DASH (Mach 0.86)
                { 0.86, 0.028,  0.048, 0.350, 0.40, 0.400,  14.0,  0.80 },

                -- MACH "WALL" (Mach 0.9+)
                { 0.9,  0.029,  0.046, 0.450, 0.60, 0.250,  12.5,  0.80 },
                
                -- Mach 0.94: Smooth drop to 0.040
                { 0.94, 0.038,  0.040, 0.500, 0.90, 0.150,  11.0,  0.72 }, 
                
                -- Mach 0.98: Hard Floor
                { 0.98, 0.065,  0.035, 0.600, 1.20, 0.100,  10.0,  0.65 },

                -- SUPERSONIC
                { 1.05, 0.120,  0.030, 0.700, 1.50, 0.050,  9.0,   0.55 },
                { 1.2,  0.150,  0.025, 0.800, 1.80, 0.020,  8.5,   0.45 },
			}, 
		}, 
		...
		engine = {
			...
			dpdh_m				= 1500,				-- [N/km] Thrust loss/km (Dry)
			dpdh_f				= 1900,				-- [N/km] Thrust loss/km (AB)

			table_data = {
				-- Klimov VK-1F Thrust Table in Newtons
				-- Dry: ~25.5 kN (26000 N), Wet: ~33.1 kN (33800 N)
				--	M	Pmax (N)	Pfor (N)	
				{ 0.0,	26000,		33800	}, -- Static Thrust (Reference)
				{ 0.1,	25800,		33600	},
				{ 0.2,	25600,		33400	}, -- Slight dip due to intake efficiency
				{ 0.3,	25500,		33200	},
				{ 0.4,	25500,		33200	}, -- Thrust stays flat in dogfight speeds
				{ 0.5,	25600,		33400	},
				{ 0.6,	25800,		33800	},
				{ 0.7,	26200,		34500	}, -- Ram effect starts kicking in
				{ 0.8,	26800,		35500	},
				...
				-- Supersonic Drop-off (Intake choke)
				{ 1.0,	27000,		36000	}, -- Thrust drops as shockwaves block intake
				{ 1.1,	26000,		35000	},
			},
		}, 
	},
	...
	Guns = {
		...
		nr23({	-- PORT FRONT 23mm
			...
			mixes				= {{2,1,1}},							-- HEI, AP, AP, [... recycle]
			ejector_pos			= {-2.35, -1.108, -0.24},
			ejector_dir 		= {0, 1/2, 0},
		}),	
		nr23({	-- PORT REAR 23mm
			muzzle_pos			= {4.1255, -1.034, -0.2784},
			mixes				= {{1,2,1}},							-- AP, HEI, AP, [... recycle]
